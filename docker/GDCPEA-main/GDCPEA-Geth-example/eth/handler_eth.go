...
func (h *ethHandler) handleHeaders(peer *eth.Peer, headers []*types.Header) error {
	...
	if filter {
		...
		GDCPEA.WriteEvalLog(",received block header,null,"+headers[0].TxHash.String()+","+strconv.FormatUint(headers[0].Number.Uint64(), 10))
		log.Debug("received header header","hash",headers[0].TxHash,"number",headers[0].Number.Uint64())
